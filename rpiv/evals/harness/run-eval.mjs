...
/** Copy the produced .md AND its sibling review.json (when present) into outputs/, so
 * the grader can read the canonical review.json alongside the rendered .md. The .md is
 * the primary artifact; the .json rides beside it (present-.md/absent-.json falls to
 * the grader's markdown path). */
export function copyArtifactOutputs(artifact, outputsDir) {
	cpSync(artifact, join(outputsDir, "artifact.md"));
	const sibling = artifact.replace(/\.md$/, ".json");
	if (existsSync(sibling)) cpSync(sibling, join(outputsDir, "artifact.json"));
}
...
function main() {
	...
	if (caseSpec.output === "agent") {
		...
		// Copy BOTH the .md and its sibling review.json (when present) into outputs/ so the
		// grader can field-assert over the canonical review.json. The no-artifact gate below
		// and the `.md`-primary discovery filter above are unchanged.
		if (artifact) copyArtifactOutputs(artifact, join(rundir, "outputs"));
	}
	writeFileSync(
