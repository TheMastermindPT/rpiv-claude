...
**Changes**: NEW — pure coach-facing summary formatter. Reports the count and the largest single reduction across the recorded clamps.
...
export function formatClampSummary(entries: LoadCapAuditEntry[]): string {
  ...
  const maxDeltaKg = entries[0].proposedLoadKg - entries[0].cappedLoadKg;
...
- **Behavior**: the coach summary reports the count and the largest reduction.
  - Test: `formatClampSummary reports count and largest reduction` -> `lib/generation/load-cap-audit/format.test.ts`
