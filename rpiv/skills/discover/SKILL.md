---
name: discover
description: Extract feature intent and requirements by interviewing the developer one question at a time, reaching mutual consensus, then synthesize a Feature Requirements Document at .rpiv/artifacts/discover/. Use as the pipeline's entry point whenever the user wants to scope a feature, figure out or capture requirements, define the problem and success criteria, or stress-test a fuzzy feature idea BEFORE any codebase research or design — e.g. 'help me figure out what I actually want to build', 'interview me about the requirements', 'turn this vague idea into a spec'. Adapts to the developer's fluency, glosses jargon, ends with a teach-back consensus check. NOT for understanding existing code (that's research), comparing solution options (explore), or designing the architecture (design). The FRD's Decisions block feeds research and propagates into design.
