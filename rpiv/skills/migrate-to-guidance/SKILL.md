...
You are tasked with migrating a project's existing `CLAUDE.md` files (typically created by `/rpiv:annotate-inline`) into the `.rpiv/guidance/` system.
...
4. **Post-migration:**
   ...
   - Print the closing footer (verbatim, with placeholders filled):
     ...
     💬 Follow-up: describe targeted edits in chat; re-run `/rpiv:migrate-to-guidance` with different flags (`--force`, `--delete-originals`) for a different migration shape.

     **Next step:** `/rpiv:annotate-guidance` — refresh or extend annotations now that the guidance tree owns them (skip if no further annotation is planned).
