...
5. **No arguments provided**:
   ...
   1. A feature name: `/rpiv:write-test-cases Order Management`
   2. A component path: `/rpiv:write-test-cases src/orders/`
   3. A feature slug: `/rpiv:write-test-cases order-management`
   4. A _meta.md path: `/rpiv:write-test-cases .rpiv/test-cases/orders/_meta.md`

   Add instructions after the feature: `/rpiv:write-test-cases Order Management focus on refund edge cases`
   ```
   Then wait for input.
...
- **When to re-invoke instead.** Re-run `/rpiv:write-test-cases <feature>` for a different feature; for the same feature, prefer in-place edits. The previous block's `Next step:` stays valid.
