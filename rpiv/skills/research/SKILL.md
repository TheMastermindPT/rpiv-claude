...
3. **Structural pre-sweep (tool-gated), then dispatch the scope-tracer agent.** Seed scope-tracer with DIGESTED structural anchors when the tools are present:
   ...
   - **CodeScene systemic seed (tool-gated, optional).** When the topic is code-health / tech-debt / hotspot / ownership-shaped AND a paid CodeScene project is attached (`mcp__codescene__list_technical_debt_hotspots_for_project` / `mcp__codescene__code_ownership_for_path` available to you and authorized — they 403 on a standalone license), fold a DIGESTED seed into the anchors: top hotspots (`path — health, churn`) and the target area's likely owners. Same no-raw-dump discipline — digest rows only. If the tools are unavailable/unauthorized or the topic is not debt/ownership-shaped, omit silently — the structural pre-sweep + scope-tracer's own sweep are the fallback.
...
💬 Follow-up: describe the change in chat to append a timestamped Follow-up section to this artifact. Re-run `/rpiv:research` for a fresh artifact.

**Next step (choose one):**
- `/rpiv:design .rpiv/artifacts/research/{filename}.md` — iterative design with vertical-slice decomposition (produces design artifact for plan)
- `/rpiv:blueprint .rpiv/artifacts/research/{filename}.md` — lightweight fast path for smaller tasks; combined design + phased plan in one pass (produces implement-ready plan directly)
