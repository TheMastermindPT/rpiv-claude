...
1. **Run quantified metrics** on the resolved target:
   ...
   - `---dup-candidates---` (`fileA | fileB  shared=N  overlap=R  tokenSim=R  via=...`) — **content-based** duplication (D) pre-signal, file PAIRS. Two signals: `overlap` = shared normalized LINES / min(|A|,|B|) (literal copy-paste); `tokenSim` = token-MinHash Jaccard (STRUCTURAL / Type-2 clones — same shape, renamed identifiers/literals). `via` is `lines` | `structural` | `both`. A pair flags when either signal clears threshold. Grounded in content, never filename coincidence; the D lens confirms semantics.
...
Dup-candidates: {fileA | fileB  shared  overlap  tokenSim  via pairs — literal + structural (Type-2) clones}
...
- **D — Semantic duplication / parallel implementations** (per `dup-candidates` pair): `codebase-pattern-finder` — "For each candidate PAIR (`{a}`, `{b}`) — {shared} shared lines, overlap {overlap}, tokenSim {tokenSim}, via {via} — read both ends and decide genuine redundancy vs distinct concern. `via=lines`/`both` = literal copy-paste (anchor on overlap + shared-line count); `via=structural` = a Type-2 clone with the SAME shape but renamed identifiers/literals (few identical lines — anchor on tokenSim; read for matching control flow, not matching text). For real duplication emit both `file:line` ranges with verbatim lines, the divergence, which is canonical, why it is slop. Confirm it is real shared logic, not coincidental boilerplate." Then, for the single strongest pair, `peer-comparator` — "Pair: (`{a}`, `{b}`). Emit one row per public invariant: Mirrored / Missing / Diverged / Intentionally-absent, with verbatim cites. No prose, no severity."
