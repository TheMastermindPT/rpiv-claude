...
1. **No argument provided** — full checkpoint mode:
   ...
   `/rpiv:frontend-design`                — full aesthetic checkpoint (scan + interview + guidelines)
   `/rpiv:frontend-design --headless`     — scan-only: inject style findings without interview
   ```
   Then wait for input.
